/**
 * \file sig_faest_256s.c
 * \brief Implementation of OQS_SIG wrapper for FAEST-256s
 */
...
OQS_SIG *OQS_SIG_faest_256s_new(void) {
	...
	sig->method_name = OQS_SIG_alg_faest_256s;
	sig->alg_version = "1.0";

	sig->claimed_nist_level = 5;
	sig->euf_cma = true;

	sig->length_public_key = OQS_SIG_faest_256s_length_public_key;
	sig->length_secret_key = OQS_SIG_faest_256s_length_secret_key;
	sig->length_signature = OQS_SIG_faest_256s_length_signature;

	sig->keypair = (OQS_STATUS (*)(uint8_t *, uint8_t *)) OQS_SIG_faest_256s_keypair;
	sig->sign = (OQS_STATUS (*)(uint8_t *, size_t *, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_faest_256s_sign;
	sig->verify = (OQS_STATUS (*)(const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_faest_256s_verify;
	sig->sign_with_ctx_str = (OQS_STATUS (*)(uint8_t *, size_t *, const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_faest_256s_sign_with_ctx_str;
	sig->verify_with_ctx_str = (OQS_STATUS (*)(const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_faest_256s_verify_with_ctx_str;
...
OQS_API OQS_STATUS OQS_SIG_faest_256s_sign(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *secret_key) {
	...
	memcpy(signature, signed_message + signed_message_len - CRYPTO_BYTES, CRYPTO_BYTES);
...
OQS_API OQS_STATUS OQS_SIG_faest_256s_sign_with_ctx_str(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *ctx_str, size_t ctx_str_len, const uint8_t *secret_key) {
	...
	return OQS_SIG_faest_256s_sign(signature, signature_len, message, message_len, secret_key);
}
...
OQS_API OQS_STATUS OQS_SIG_faest_256s_verify_with_ctx_str(const uint8_t *message, size_t message_len, const uint8_t *signature, size_t signature_len, const uint8_t *ctx_str, size_t ctx_str_len, const uint8_t *public_key) {
	...
	return OQS_SIG_faest_256s_verify(message, message_len, signature, signature_len, public_key);
}
