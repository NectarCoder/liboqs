/**
 * \file sig_mirath_tcith_1a_short.c
 * \brief OQS glue for MIRATH-TCITH-1A-SHORT
 */
...
OQS_API OQS_STATUS OQS_SIG_mirath_tcith_1a_short_keypair(uint8_t *public_key, uint8_t *secret_key) {
	...
	if (crypto_sign_keypair(public_key, secret_key) != 0) {
		memset(public_key, 0, OQS_SIG_mirath_tcith_1a_short_length_public_key);
		memset(secret_key, 0, OQS_SIG_mirath_tcith_1a_short_length_secret_key);
		return OQS_ERROR;
	}
...
OQS_API OQS_STATUS OQS_SIG_mirath_tcith_1a_short_sign(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *secret_key) {
	...
	if (ret != 0 || produced_len != signed_message_len) {
		...
		memset(signature, 0, OQS_SIG_mirath_tcith_1a_short_length_signature);
		return OQS_ERROR;
	}
