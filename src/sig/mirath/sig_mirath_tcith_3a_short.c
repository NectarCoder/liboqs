/**
 * \file sig_mirath_tcith_3a_short.c
 * \brief OQS glue for MIRATH-TCITH-3A-SHORT
 */
...
#include "mirath_tcith_3a_short/api.h"
...
OQS_SIG *OQS_SIG_mirath_tcith_3a_short_new(void) {
	...
	sig->method_name = OQS_SIG_alg_mirath_tcith_3a_short;
	...
	sig->length_public_key = OQS_SIG_mirath_tcith_3a_short_length_public_key;
	sig->length_secret_key = OQS_SIG_mirath_tcith_3a_short_length_secret_key;
	sig->length_signature = OQS_SIG_mirath_tcith_3a_short_length_signature;

	sig->keypair = (OQS_STATUS (*)(uint8_t *, uint8_t *)) OQS_SIG_mirath_tcith_3a_short_keypair;
	sig->sign = (OQS_STATUS (*)(uint8_t *, size_t *, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_mirath_tcith_3a_short_sign;
	sig->verify = (OQS_STATUS (*)(const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_mirath_tcith_3a_short_verify;
	sig->sign_with_ctx_str = (OQS_STATUS (*)(uint8_t *, size_t *, const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_mirath_tcith_3a_short_sign_with_ctx_str;
	sig->verify_with_ctx_str = (OQS_STATUS (*)(const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_mirath_tcith_3a_short_verify_with_ctx_str;
...
OQS_API OQS_STATUS OQS_SIG_mirath_tcith_3a_short_keypair(uint8_t *public_key, uint8_t *secret_key) {
	...
	if (crypto_sign_keypair(public_key, secret_key) != 0) {
		memset(public_key, 0, OQS_SIG_mirath_tcith_3a_short_length_public_key);
		memset(secret_key, 0, OQS_SIG_mirath_tcith_3a_short_length_secret_key);
		return OQS_ERROR;
	}
...
OQS_API OQS_STATUS OQS_SIG_mirath_tcith_3a_short_sign(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *secret_key) {
	...
	if (ret != 0 || produced_len != signed_message_len) {
		...
		memset(signature, 0, OQS_SIG_mirath_tcith_3a_short_length_signature);
		return OQS_ERROR;
	}
...
OQS_API OQS_STATUS OQS_SIG_mirath_tcith_3a_short_sign_with_ctx_str(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *ctx_str, size_t ctx_str_len, const uint8_t *secret_key) {
	...
	return OQS_SIG_mirath_tcith_3a_short_sign(signature, signature_len, message, message_len, secret_key);
}
...
OQS_API OQS_STATUS OQS_SIG_mirath_tcith_3a_short_verify_with_ctx_str(const uint8_t *message, size_t message_len, const uint8_t *signature, size_t signature_len, const uint8_t *ctx_str, size_t ctx_str_len, const uint8_t *public_key) {
	...
	return OQS_SIG_mirath_tcith_3a_short_verify(message, message_len, signature, signature_len, public_key);
}
