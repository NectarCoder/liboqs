...
OQS_SIG *OQS_SIG_less_400_220_new(void) {
	...
	sig->method_name = OQS_SIG_alg_less_400_220;
	sig->alg_version = "1.2";
	...
	sig->length_public_key = OQS_SIG_less_400_220_length_public_key;
	sig->length_secret_key = OQS_SIG_less_400_220_length_secret_key;
	sig->length_signature = OQS_SIG_less_400_220_length_signature;

	sig->keypair = OQS_SIG_less_400_220_keypair;
	sig->sign = OQS_SIG_less_400_220_sign;
	sig->verify = OQS_SIG_less_400_220_verify;
	sig->sign_with_ctx_str = OQS_SIG_less_400_220_sign_with_ctx_str;
	sig->verify_with_ctx_str = OQS_SIG_less_400_220_verify_with_ctx_str;
...
OQS_API OQS_STATUS OQS_SIG_less_400_220_sign_with_ctx_str(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *ctx_str, size_t ctx_str_len, const uint8_t *secret_key) {
	if ((ctx_str != NULL && ctx_str_len > 0)) {
		return OQS_ERROR;
	}
	return OQS_SIG_less_400_220_sign(signature, signature_len, message, message_len, secret_key);
}

OQS_API OQS_STATUS OQS_SIG_less_400_220_verify_with_ctx_str(const uint8_t *message, size_t message_len, const uint8_t *signature, size_t signature_len, const uint8_t *ctx_str, size_t ctx_str_len, const uint8_t *public_key) {
	if ((ctx_str != NULL && ctx_str_len > 0)) {
		return OQS_ERROR;
	}
	return OQS_SIG_less_400_220_verify(message, message_len, signature, signature_len, public_key);
}
