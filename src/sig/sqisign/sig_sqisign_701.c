 ...
 * \brief Implementation of OQS_SIG wrapper for SQIsign-701
 */
...
static void oqs_sqisign_701_ensure_rng(void) {
...
OQS_SIG *OQS_SIG_sqisign_701_new(void) {
        ...
        sig->keypair = (OQS_STATUS (*)(uint8_t *, uint8_t *)) OQS_SIG_sqisign_701_keypair;
        sig->sign = (OQS_STATUS (*)(uint8_t *, size_t *, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_sqisign_701_sign;
        sig->verify = (OQS_STATUS (*)(const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_sqisign_701_verify;
        sig->sign_with_ctx_str = (OQS_STATUS (*)(uint8_t *, size_t *, const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_sqisign_701_sign_with_ctx_str;
        sig->verify_with_ctx_str = (OQS_STATUS (*)(const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *, size_t, const uint8_t *)) OQS_SIG_sqisign_701_verify_with_ctx_str;
...
OQS_API OQS_STATUS OQS_SIG_sqisign_701_keypair(uint8_t *public_key, uint8_t *secret_key) {
        ...
        oqs_sqisign_701_ensure_rng();

        if (crypto_sign_keypair(public_key, secret_key) != 0) {
                memset(public_key, 0, OQS_SIG_sqisign_701_length_public_key);
                memset(secret_key, 0, OQS_SIG_sqisign_701_length_secret_key);
                return OQS_ERROR;
        }
...
OQS_API OQS_STATUS OQS_SIG_sqisign_701_sign(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *secret_key) {
        ...
        if (message_len > SIZE_MAX - OQS_SIG_sqisign_701_length_signature) {
                return OQS_ERROR;
        }

        oqs_sqisign_701_ensure_rng();

        const size_t sm_len_expected = message_len + OQS_SIG_sqisign_701_length_signature;
        ...
        if (ret != 0 || sm_len != sm_len_expected) {
                memset(signature, 0, OQS_SIG_sqisign_701_length_signature);
                OQS_MEM_insecure_free(sm);
                return OQS_ERROR;
        }

        memcpy(signature, sm, OQS_SIG_sqisign_701_length_signature);
