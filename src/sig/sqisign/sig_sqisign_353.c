 ...
 * \brief Implementation of OQS_SIG wrapper for SQIsign-353 (NIST Level 1)
 */
...
OQS_SIG *OQS_SIG_sqisign_353_new(void) {
	...
	sig->keypair = OQS_SIG_sqisign_353_keypair;
	sig->sign = OQS_SIG_sqisign_353_sign;
	sig->verify = OQS_SIG_sqisign_353_verify;
	sig->sign_with_ctx_str = OQS_SIG_sqisign_353_sign_with_ctx_str;
	sig->verify_with_ctx_str = OQS_SIG_sqisign_353_verify_with_ctx_str;

	return sig;
}

OQS_API OQS_STATUS OQS_SIG_sqisign_353_keypair(uint8_t *public_key, uint8_t *secret_key) {
	return (sqisign_lvl1_ref_sqisign_keypair(public_key, secret_key) == 0) ? OQS_SUCCESS : OQS_ERROR;
}
...
OQS_API OQS_STATUS OQS_SIG_sqisign_353_sign(uint8_t *signature, size_t *signature_len, const uint8_t *message, size_t message_len, const uint8_t *secret_key) {
	...
	uint8_t *signed_msg = OQS_MEM_malloc(message_len + OQS_SIG_sqisign_353_length_signature);
	if (signed_msg == NULL) {
		return OQS_ERROR;
	}

	int ret = sqisign_lvl1_ref_sqisign_sign(signed_msg, &signed_msg_len, message, (unsigned long long) message_len, secret_key);
	if (ret != 0) {
	...
	memcpy(signature, signed_msg, OQS_SIG_sqisign_353_length_signature);
...
OQS_API OQS_STATUS OQS_SIG_sqisign_353_verify(const uint8_t *message, size_t message_len, const uint8_t *signature, size_t signature_len, const uint8_t *public_key) {
	...
	uint8_t *signed_msg = OQS_MEM_malloc(message_len + OQS_SIG_sqisign_353_length_signature);
	uint8_t *recovered_msg = OQS_MEM_malloc(message_len + OQS_SIG_sqisign_353_length_signature);
	...
	memcpy(signed_msg, signature, OQS_SIG_sqisign_353_length_signature);
	memcpy(signed_msg + OQS_SIG_sqisign_353_length_signature, message, message_len);

	int ret = sqisign_lvl1_ref_sqisign_open(recovered_msg, &recovered_msg_len, signed_msg, message_len + OQS_SIG_sqisign_353_length_signature, public_key);
